...
//        CGPoint pointA = ccp(bodyB->GetJointList()->joint->GetAnchorA().x * PTM_RATIO, bodyB->GetJointList()->joint->GetAnchorA().y * PTM_RATIO);
//        CGPoint pointB = ccp(bodyB->GetJointList()->joint->GetAnchorB().x * PTM_RATIO, bodyB->GetJointList()->joint->GetAnchorB().y * PTM_RATIO);
...
-(void)createRope:(CGPoint)pointA pointB:(CGPoint)pointB {
	...
	int segmentFactor = 5; //increase value to have less segments per rope, decrease to have more segments
...
- (void) drawRope 
{
    GLfloat vertices[numPoints];
    GLubyte squareColors[numPoints * 2];
    ...
    for (int i = 0; i < (numPoints * 2) - 1; i += 4)
    {
        squareColors[i] = 255;
        squareColors[i + 1] = 0;
        squareColors[i + 2] = 0;
        squareColors[i + 3] = 255;
    }
    
    //CCLOG(@"%i", numPoints);
    
    //GLubyte squareColors[8];
//    squareColors[0] = 255;
//    squareColors[1] = 0;
//    squareColors[2] = 0;
//    squareColors[3] = 255;
...
//    squareColors[7] = 255;
    ...
    glDrawArrays(GL_LINE_STRIP, 0, numPoints / 2);
